...
function AdjustStock() {
  ...
  const currentType =
    adjustmentTypes.find((t) => t.value === adjustType) || adjustmentTypes[0];
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentType.direction === "out" && qty > selectedProduct.stock) {
      toast.error(
        `Cannot remove ${qty} units. Only ${selectedProduct.stock} in stock.`,
      );
      return;
    }
    ...
    setSaving(true);
    try {
      await api.post(`/products/${selectedProduct._id}/adjust`, {
      ...
      toast.success(
        `Stock adjusted: ${currentType.direction === "in" ? "+" : "-"}${qty} units of ${selectedProduct.name}`,
      );
              ...
              >
                ...
                <div
                  style={{
                    ...
                    padding: 8,
                    background:
                      currentType.direction === "in" ? "#dbeafe" : "#fee2e2",
                    borderRadius: 6,
                  }}
                >
                  <div
                    style={{
                      fontSize: "0.7rem",
                      color:
                        currentType.direction === "in" ? "#2563eb" : "#dc2626",
                    }}
                  >
                    {currentType.label}
                  </div>
            ...
            >
              {adjustmentTypes.map((t) => (
                <button
                  ...
                  type="button"
                  className={
                    adjustType === t.value ? "primary-btn" : "subtle-btn"
                  }
