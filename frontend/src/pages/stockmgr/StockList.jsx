...
function StockList() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterStatus)
      list = list.filter(
        (p) => p.status?.toLowerCase().replace(/\s+/g, "-") === filterStatus,
      );
    return list;
  ...
  return (
    <WorkspaceLayout>
      ...
      >
        ...
        <button className="subtle-btn" onClick={load}>
          Refresh
        </button>
        <button className="primary-btn" onClick={() => setShowScanner(true)}>
          Scan Product
        </button>
      </div>
      ...
      <div className="table-shell">
        <div
          ...
          style={{ gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr 1.5fr" }}
        >
          <div>Product</div>
          <div>SKU</div>
          <div>Category</div>
          ...
          {loading ? (
            ...
            filtered.map((p) => (
              ...
              >
                ...
                <div>
                  <span
                    className={`status-pill ${p.stock === 0 ? "status-Cancelled" : p.stock <= p.reorderLevel ? "status-Pending" : "status-Received"}`}
                  >
                    {p.status}
                  </span>
                </div>
                ...
                >
                  ...
                  <button
                    className="text-action"
                    onClick={() =>
                      navigate("/sm/adjust-stock", { state: { product: p } })
                    }
                  >
                    Adjust
                  </button>
      ...
      {barcodeModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setBarcodeModal(null);
          }}
        >
                    ...
                    if (e.key === "Enter" && e.target.value) {
                      try {
                        const res = await api.get(
                          `/products/barcode/${e.target.value}`,
                        );
                        if (res.data)
                          navigate("/sm/adjust-stock", {
